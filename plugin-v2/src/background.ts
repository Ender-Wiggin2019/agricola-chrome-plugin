  ...
  if (message.action === "openSidePanel") {
    ...
    if (windowId !== undefined) {
      ...
      chrome.sidePanel.open({ windowId })
        .then(() => {
          console.log("[Agricola Tutor] Side panel opened successfully from content script")
          sendResponse({ success: true })
        })
        .catch((error) => {
          console.error("[Agricola Tutor] Failed to open side panel from content script:", error)
          console.error("[Agricola Tutor] Error details:", JSON.stringify(error, Object.getOwnPropertyNames(error)))
    ...
    chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
      ...
      if (windowId !== undefined) {
        ...
        try {
          ...
          console.log("[Agricola Tutor] Side panel opened successfully from popup")
          sendResponse({ success: true })
        } catch (error: any) {
          console.error("[Agricola Tutor] Failed to open side panel from popup:", error)
        ...
        console.error("[Agricola Tutor] No active tab found or windowId is undefined")
